...
>(({ className, label, ...props }, ref) => {
  const labelId = label ? `slider-${label.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}` : undefined;
  
  return (
    <div className="w-full">
      {label && (
        <label id={labelId} className="block text-sm font-medium text-yellow-400 mb-2">
          {label}
        </label>
      )}
